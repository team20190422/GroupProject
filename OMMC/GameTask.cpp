...
int GameTask::GameInit(void)
{
	...
	(*obstracle)->init("image/meteo.png", VECTOR2(64 / 2, 32 / 1),VECTOR2(2,1), VECTOR2(1, 0), 1.0f);
	back = new BackGround();
	return 0;
}
...
int GameTask::GameMain(void)
{
	...
	//プレイヤーのスピードが0より小さいならバック画面のスクロールを逆（下から上）にする
	for (auto itr : objList) {
		for (auto itrBG : backVec) {
			if ((itr->GetSpeed() < (float)0.0))/*||())*/{
				itrBG->SetFlag(true);
				DrawFormatString(10, 220, 0xffffff, "GetSpeed % 2f", (*player)->GetSpeed());
			}
			else if (itr->GetSpeed() > (float)1.0) {
				itrBG->SetFlag(false);
			}
		}
	}
